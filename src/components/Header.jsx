import { styles } from '../constants/styles';

export default function Header({ email, onSignOut, onSignIn, esMovil }) {
    const dynamicHeader = {
        ...styles.header,
        fontSize: esMovil ? '1.5rem' : '2.5rem'
    };

    return (
        <>
            <div style={{ position: 'absolute', top: 10, right: 10 }}>
                {email ? (
                    <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                        {!esMovil && <span style={{ fontSize: '12px', color: '#666' }}>{email}</span>}
                        <button onClick={onSignOut} style={styles.logoutBtn}>Cerrar Sesión</button>
                    </div>
                ) : (
                    <button
                        onClick={onSignIn}
                        style={{
                            ...styles.logoutBtn,
                            backgroundColor: '#4285F4',
                            display: 'flex',
                            alignItems: 'center',
                            gap: '8px',
                            fontSize: '14px',
                            padding: '8px 16px'
                        }}
                    >
                        <svg width="18" height="18" viewBox="0 0 18 18">
                            <path fill="#fff" d="M17.64 9.2c0-.637-.057-1.251-.164-1.84H9v3.481h4.844c-.209 1.125-.843 2.078-1.796 2.717v2.258h2.908c1.702-1.567 2.684-3.874 2.684-6.615z" />
                            <path fill="#fff" d="M9 18c2.43 0 4.467-.806 5.956-2.184l-2.908-2.259c-.806.54-1.837.86-3.048.86-2.344 0-4.328-1.584-5.036-3.711H.957v2.332A8.997 8.997 0 0 0 9 18z" />
                            <path fill="#fff" d="M3.964 10.706A5.41 5.41 0 0 1 3.682 9c0-.593.102-1.17.282-1.706V4.962H.957A8.996 8.996 0 0 0 0 9c0 1.452.348 2.827.957 4.038l3.007-2.332z" />
                            <path fill="#fff" d="M9 3.58c1.321 0 2.508.454 3.44 1.345l2.582-2.58C13.463.891 11.426 0 9 0A8.997 8.997 0 0 0 .957 4.962l3.007 2.332C4.672 5.164 6.656 3.58 9 3.58z" />
                        </svg>
                        Entrar con Google
                    </button>
                )}
            </div>
            <h1 style={dynamicHeader}>CryptoView <span style={{ color: '#6366f1' }}>2026</span></h1>
            {email && <p style={{ textAlign: 'center', color: '#666', marginBottom: '20px' }}>Dashboard de Criptomonedas</p>}
        </>
    );
}
